...
// ── Unsplash Rasm to'plamlari (Toifalar bo'yicha) ──
const CAT_IMAGES = {
  tarixiy: [
...
// 🌟 1-QISM: 35 TA DIQQATGA SAZOVOR JOY (Tarixiy, Tabiat, Ziyoratgoh) 🌟
const ATTRACTIONS_PART_1 = [
  {
...
// 🌟 2-QISM: 36 TA INFRATUZILMA, KASALXONA, IIB, HOKIMIYAT, BOZOR VA SUPERMARKET 🌟
const ATTRACTIONS_PART_2 = [
  {
...
// 🌟 3-QISM: 22 TA MEHMONXONA VA DAM OLISH MASKANI 🌟
const HOTELS_DATA = [
  {
...
async function seedAllDatas() {
  const mongoUri = process.env.MONGO_URI || 'mongodb://localhost:27017/tour-app';
  console.log('📡 MongoDBga ulanmoqda:', mongoUri);

  try {
    await mongoose.connect(mongoUri);
    console.log('✅ MongoDB ulanishi muvaffaqiyatli!');

    // 1. Foydalanuvchilarni tayyorlash
    ...
    if (!adminUser) {
      ...
      console.log('👤 Standart Admin yaratildi: admin@tour.uz / admin123');
    }

    let ownerUser = await User.findOne({ role: 'HOTEL_OWNER' });
    if (!ownerUser) {
      ...
      console.log('👤 Standart Hotel egasi yaratildi: owner@tour.uz / owner123');
    }

    // 2. Mavjud ma'lumotlarni tozalash
    console.log('🧹 Eski obyektlar va mehmonxonalar tozalanmoqda...');
    await Attraction.deleteMany({});
    await Hotel.deleteMany({});

    // 3. 71 ta Obyektni shakllantirish (360 Video url bo'sh holda)
    const allRawAttractions = [...ATTRACTIONS_PART_1, ...ATTRACTIONS_PART_2];
    const attractionDocs = allRawAttractions.map(item => {
      const catImages = CAT_IMAGES[item.category] || CAT_IMAGES.tarixiy;
      return {
        name: item.name,
        ...
        video360: { url: '', type: 'youtube', captioned: false }, // 360 video bo'sh (kerak emas)
        images: catImages,
        approved: true,
    ...
    const insertedAttractions = await Attraction.insertMany(attractionDocs);
    console.log(`🎉 Jami ${insertedAttractions.length} ta Obyekt (Diqqatga sazovor + Infratuzilma) muvaffaqiyatli kiritildi!`);

    // 4. 22 ta Mehmonxonani shakllantirish
    const hotelDocs = HOTELS_DATA.map(h => {
      const catImages = CAT_IMAGES[h.category] || CAT_IMAGES.hotel;
      return {
        name: h.name,
        district: h.district,
        city: h.district,
        ...
        totalRooms: 10,
        rooms: [
          { name: 'Standart Xona', capacity: 2, pricePerNight: h.basePricePerNight, roomType: 'Double Room', category: 'Standard' },
          { name: 'Lyuks Xona', capacity: 2, pricePerNight: Math.round(h.basePricePerNight * 1.4), roomType: 'Double Room', category: 'Deluxe' }
        ]
      };
    });

    const insertedHotels = await Hotel.insertMany(hotelDocs);
    console.log(`🏨 Jami ${insertedHotels.length} ta Mehmonxona va Dam olish maskani muvaffaqiyatli kiritildi!`);

    console.log('\n📊 XULOSA:');
    console.log(`🏛️ Diqqatga sazovor joylar & Infratuzilma: ${insertedAttractions.length} ta`);
    console.log(`🏨 Mehmonxonalar: ${insertedHotels.length} ta`);
    console.log('✅ Barcha ma\'lumotlar toifalari bo\'yicha o\'ziga xos ikonkalar va ranglar bilan biriktirildi!');
